...
type AdminRequestShortcode struct {
...
func AdminReportPieService(r *http.Request) map[string][]dataSummary {
	var output = map[string][]dataSummary{
		"data-summary": {},
	}

	var adminRequestShortcode AdminRequestShortcode
	err := json.NewDecoder(r.Body).Decode(&adminRequestShortcode)
	if err != nil {
		fmt.Println("admin_report_overall_service : Invalid JSON #1")
		return output
	}
	...
	for _, sc := range adminRequestShortcode.ListShortcode {
